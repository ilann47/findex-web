...
import { ConcreteTensometerReading } from '../../reading/stress-meter'
...
interface Props {
	calibration?: ConcreteTensometerCalibrationType
	showDate?: boolean
	isLoading?: boolean
	adornment?: ReactNode
	form?: boolean
...
export const ConcreteTensometerCalibration = ({
	calibration,
	showDate = true,
	adornment,
	isLoading = false,
	...
	const { remove } = useMutate({ endpoint: `${ENDPOINTS.CALIBRATION}/${ENDPOINTS.STRESS_METER}` })
	...
	return (
		<LoadingContext.Provider value={isLoading}>
			...
			<GridContainer>
				...
				<Field component='grid' labelId='instrument.calibration.td80'>
					{calibration?.td80}
				</Field>
				<Field component='grid' labelId='instrument.calibration.ex'>
					{calibration?.ex}
				</Field>
				<Field component='grid' labelId='instrument.calibration.ac'>
					{calibration?.ac}
				</Field>
			...
			{calibration?.prioriReading && (
				<>
					...
					<GridContainer>
						<ConcreteTensometerReading reading={calibration.prioriReading} />
					</GridContainer>
				</>
			)}
			...
			{calibration?.posterioriReading && (
				<>
					...
					<GridContainer>
						<ConcreteTensometerReading reading={calibration.posterioriReading} />
					</GridContainer>
				</>
			)}
