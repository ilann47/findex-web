...
export const ElectricJointMeterCalibrationCalibrationHistory = ({ instrumentId }: Props) => {
	const { data: calibrations, isLoading } = useGetAll<ElectricJointMeterCalibrationType>({
	...
	if (isLoading)
		return (
			<CalibrationHistoryLoading>
				<ElectricJointMeterCalibration isLoading showDate={false} />
			</CalibrationHistoryLoading>
		)
	...
	return (
		<Stack direction='row' gap={3}>
			...
			<StyledContainer height='max-content'>
				{selectedCalibration && (
					<ElectricJointMeterCalibration calibration={selectedCalibration} showDate={false} />
				)}
			</StyledContainer>
		</Stack>
	)
