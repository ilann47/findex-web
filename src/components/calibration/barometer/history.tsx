...
export const BarometerCalibrationHistory = ({ instrumentId }: Props) => {
	const { data: calibrations, isLoading } = useGetAll<BarometerCalibrationType>({
	...
	if (isLoading)
		return (
			<CalibrationHistoryLoading>
				<BarometerCalibration isLoading showDate={false} />
			</CalibrationHistoryLoading>
		)
		...
		<>
			<Stack direction='row' gap={3}>
				...
				<StyledContainer height='max-content'>
					{selectedCalibration && <BarometerCalibration calibration={selectedCalibration} showDate={false} />}
				</StyledContainer>
			</Stack>
		</>
	)
