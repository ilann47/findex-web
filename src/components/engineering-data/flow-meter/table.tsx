...
import { EngineeringData, FlowMeterEngineeringData } from '@/schemas/engineering-data'
...
export const FlowMeterEngineeringDataTable = ({ instrument, getRowLink }: Props) => {
	...
	const {
		...
		totalElements,
		isLoading,
	} = useGetPageable<FlowMeterEngineeringData>({
		endpoint: `${ENDPOINTS.ENGINEERING_DATA}/${ENDPOINTS.FLOW_METER}`,
		requestParams: { ...requestParams, sort: DATE_SORT },
	})

	const columnHelper = createColumnHelper<FlowMeterEngineeringData>()

	const columns: ColumnDef<FlowMeterEngineeringData>[] = [
		...
		columnHelper.accessor('waterFlow', {
			id: 'waterFlow',
			header: formatEngineeringValue(
				formatMessage('instrument.engineering-value.water-flow.name'),
				formatMessage('instrument.engineering-value.water-flow.uom')
			),
		}),
	] as ColumnDef<FlowMeterEngineeringData>[]
