...
import { ExtensometerReading } from '@/schemas/reading'
...
export const ExtensometerReadingsTable = ({ instrument }: Props) => {
	...
	const {
		...
		totalElements,
		isLoading,
	} = useGetPageable<ExtensometerReading>({
		endpoint: `${ENDPOINTS.READING}/${ENDPOINTS.EXTENSOMETER}`,
		requestParams: { ...requestParams, sort: DATE_SORT },
	})

	const columnHelper = createColumnHelper<ExtensometerReading>()

	const columns: ColumnDef<ExtensometerReading>[] = [
	...
	] as ColumnDef<ExtensometerReading>[]
