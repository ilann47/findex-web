...
import { ConcreteTensometerReading } from '@/schemas/reading'
...
export const ConcreteTensometerReadingsTable = ({ instrument }: Props) => {
	...
	const {
		...
		totalElements,
		isLoading,
	} = useGetPageable<ConcreteTensometerReading>({
		endpoint: `${ENDPOINTS.READING}/${ENDPOINTS.STRESS_METER}`,
		requestParams: { ...requestParams, sort: DATE_SORT },
	})

	const columnHelper = createColumnHelper<ConcreteTensometerReading>()

	const columns: ColumnDef<ConcreteTensometerReading>[] = [
	...
	] as ColumnDef<ConcreteTensometerReading>[]
