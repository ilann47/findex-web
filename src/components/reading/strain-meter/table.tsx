...
import { DeformimeterReading } from '@/schemas/reading'
...
export const DeformimeterReadingsTable = ({ instrument }: Props) => {
	...
	const {
		...
		totalElements,
		isLoading,
	} = useGetPageable<DeformimeterReading>({
		endpoint: `${ENDPOINTS.READING}/${ENDPOINTS.STRAIN_METER}`,
		requestParams: { ...requestParams, sort: DATE_SORT },
	})

	const columnHelper = createColumnHelper<DeformimeterReading>()

	const columns: ColumnDef<DeformimeterReading>[] = [
	...
	] as ColumnDef<DeformimeterReading>[]
