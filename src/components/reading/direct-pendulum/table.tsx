...
import { DirectPendulumReading } from '@/schemas/reading'
...
export const DirectPendulumReadingsTable = ({ instrument }: Props) => {
	...
	const {
		...
		totalElements,
		isLoading,
	} = useGetPageable<DirectPendulumReading>({
		endpoint: `${ENDPOINTS.READING}/${ENDPOINTS.DIRECT_PENDULUM}`,
		requestParams: { ...requestParams, sort: DATE_SORT },
	})

	const columnHelper = createColumnHelper<DirectPendulumReading>()

	const columns: ColumnDef<DirectPendulumReading>[] = [
	...
	] as ColumnDef<DirectPendulumReading>[]
